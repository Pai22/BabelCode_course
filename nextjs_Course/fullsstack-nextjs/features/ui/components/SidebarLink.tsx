...
const SidebarLink = ({ href, title, Icon }: SidebarLinkProps) => {
  ...
  return (
    <Link href={href} className={linkClasses}>
      ...
      <span className="font-xs absolute top-1/2 left-full z-50 ml-4 -translate-y-1/2 rounded bg-gray-900 px-2 py-1.5 text-xs font-medium text-white opacity-0 group-hover:opacity-100">
        {title}
      </span>
    </Link>
  );
